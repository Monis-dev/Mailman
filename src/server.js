...
app.get("/v1/jobs/:id", async (req, res) => {
  try {
    ...
    const response = await pool.query("SELECT * FROM jobs WHERE id = $1", [jobId])
...
app.post("/v1/jobs", rateLimiter, Authorize ,async (req, res) => {
  try {
    ...
    if(!isValidWebhookUrl(target_url)) {
      return res.status(400).json({error: "Invalid target_url: Private or metadata addresses are prohibited"})
    }
